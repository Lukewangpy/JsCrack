...
function getdata(slide, token, X) {
    var traceData = [];
    for (var i = 0; i < slide.length; i++) {
        traceData.push(YYY(token, slide[i] + ""));
    }
    var n = sample(traceData, 50);
    var r = eypt(YYY(token, parseInt(X, 10) / 320 * 100 + ""));
    var data = JSON.stringify({
        d: eypt(n.join(":")),
        m: "",
        p: r,
        ext: eypt(YYY(token, 1 + "," + traceData.length))
    });
    return data;
}

function getValidate(token, x) {
    var max = parseInt(x, 10);
    x = Math.round(parseInt(x, 10) / 100 + "");
	...
	while (i < max) {
		...
		if (RandomNum(1, 10) % 6 == 3 && i < max - 5) {
		    ...
		    i += 3;
		} else if ((RandomNum(1, 10) % 10 == 1 && i < max - 5)) {
            i += 3;
        } else {
            i += 1;
        }
    ...
    var n = _e.sample(traceData, 50);
    var data = JSON.stringify({
        "d": eypt(n.join(":")),
        "m": "",
        "p": eypt(_t.xor_encode(token, parseInt(x, 10) / 220 * 100 + "")),
        "ext": eypt(_t.xor_encode(token, "1," + traceData.length))
    });
